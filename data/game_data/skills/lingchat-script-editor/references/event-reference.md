...
本文件为 LingChat 脚本引擎全部 16 种事件类型的权威参考，字段与默认值均取自 Rust 源码：
`src-tauri/src/ai_service/game_system/script_engine/events/events/*.rs`

> 注意：官方文档《LingChat 剧本创作指南 / 事件大全》中有几处旧写法与源码不一致，**以本文件为准**：
> - 背景特效事件 type 是 `background_effect`（下划线），**不是** `backgroundEffect`
> - 背景音乐事件 type 是 `music`，**不是** `background_music`
> - 章节结束事件 type 是 `chapter_end`，**不是** `charpter_end`
> - `free_dialogue` 的 AI 提示字段是 `prompt`，**不是** `dialog_prompt`
> - `background` 事件没有 `duration` 字段，只有 `transition`（默认 1.0）
...
|------|------|------|------|
| `name` | ✅ | string | 变量名 |
| `value` | ✅ | bool/int/float/string | 支持 `random(min,max)`、`null` |
| `op` | 否 | string | `=`（赋值）/ `+=`（加）/ `-=`（减），默认 `=` |
