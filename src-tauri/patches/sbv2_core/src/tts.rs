...
pub struct TTSModelHolder {
    ...
    /// 推理硬件设备（热切换：unload 后用新设备重建 holder）。
...
impl TTSModelHolder {
    ...
    pub fn load_aivmx<I: Into<TTSIdent>, P: AsRef<[u8]>>(
        ...
        if self.find_model(ident.clone()).is_err() {
            ...
            let model = model::load_model_with_device(&aivmx_bytes, false, self.device)?;
    ...
    pub fn load<I: Into<TTSIdent>, P: AsRef<[u8]>>(
        ...
        if self.find_model(ident.clone()).is_err() {
            ...
            self.models.push(TTSModel {
                vits2: if load {
                    Some(model::load_model_with_device(&vits2_bytes, false, self.device)?)
                } else {
                    None
                },
    ...
    fn find_and_load_model<I: Into<TTSIdent>>(&mut self, ident: I) -> Result<bool> {
        ...
        let s = model::load_model_with_device(&bytes, false, self.device)?;
