...
mod tests {
    ...
    fn todo(id: i64, text: &str) -> TodoItem {
        TodoItem {
            id,
            text: text.into(),
            ..Default::default()
    ...
    fn settings_with_groups(groups: &[(&str, Vec<TodoItem>)]) -> UserScheduleSettings {
    ...
    #[test]
    fn new_todo_ids_are_unique_across_groups() {
        let settings =
            settings_with_groups(&[("work", vec![todo(2, "A")]), ("home", vec![todo(7, "B")])]);
        assert_eq!(next_todo_id(&settings).unwrap(), 8);
    }

    #[test]
    fn duplicate_legacy_ids_require_group() {
        let settings =
            settings_with_groups(&[("work", vec![todo(1, "A")]), ("home", vec![todo(1, "B")])]);
        let error = locate_todo_group(&settings, 1, None).unwrap_err();
        assert!(error.contains("多个分组"));
        assert_eq!(
            locate_todo_group(&settings, 1, Some("home")).unwrap(),
            "home"
        );
    }

    #[test]
    fn checked_priority_rejects_overflow() {
        let args = json!({"priority": i64::MAX});
        let error =
            optional_i32(args.as_object().unwrap(), "priority", "schedule_add_todo").unwrap_err();
        assert!(error.to_string().contains("超出 i32 范围"));
    }
}
