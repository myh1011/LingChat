<template>
  <div class="flex w-full min-h-0 flex-1 gap-4">
    <!-- 左栏：会话列表 -->
    <aside class="flex w-[230px] min-h-0 shrink-0 flex-col gap-3">
      <button
        class="inline-flex items-center justify-center gap-1 rounded-xl border border-brand/45 bg-brand/14 px-3 py-2 text-[0.82rem] text-brand transition-all duration-200 hover:bg-brand/24 disabled:opacity-50"
        :disabled="store.loading"
        @click="store.createConversation()"
      >
        <span class="text-[1rem] leading-none">＋</span> 新建对话
      </button>
        ...
        <button
          ...
          :class="
            c.id === store.currentId
              ...
              : 'border-white/10 bg-white/6 hover:border-brand/40 hover:bg-white/10'
          "
          @click="store.switchConversation(c.id)"
        >
          <div class="flex items-center justify-between gap-2">
            <span class="truncate text-[0.8rem] text-white/85">{{ c.title || `对话 ${c.id}` }}</span>
            <span
              class="opacity-0 transition-opacity group-hover:opacity-100"
              :title="'删除此对话'"
              @click.stop="removeConversation(c)"
            >
              <Icon icon="close" :size="13" class="cursor-pointer text-white/50 hover:text-red-300" />
            </span>
          </div>
          <div v-if="c.scriptKey" class="mt-1 truncate font-mono text-[0.66rem] text-brand/70">
            📕 {{ c.scriptKey }}
          </div>
        </button>
      </div>
    ...
    <!-- 右栏：聊天 -->
    <div class="flex min-w-0 min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-white/10 bg-white/4">
      <!-- 消息区 -->
      <div ref="scroller" class="flex min-h-0 flex-1 flex-col gap-3 overflow-y-auto px-4 py-4">
        <div
          v-if="store.loading"
          class="py-10 text-center text-[0.82rem] text-white/40"
        >
          加载中…
        </div>
        <div
        ...
        <template v-else>
          <template v-for="item in store.items" :key="item.id">
            ...
            <div v-if="item.role === 'user'" class="flex justify-end">
              <div
                class="max-w-[76%] whitespace-pre-wrap break-words rounded-2xl rounded-tr-sm border border-brand/40 bg-brand/12 px-3.5 py-2.5 text-[0.86rem] leading-relaxed text-white/90"
              >
                {{ item.content }}
              </div>
            </div>
            ...
            <div v-else class="flex flex-col gap-2">
              <div v-for="(round, i) in item.rounds" :key="i" class="flex flex-col gap-2">
                <div
                  v-if="round.content"
                  class="max-w-[92%] whitespace-pre-wrap break-words rounded-2xl rounded-tl-sm border border-white/10 bg-white/8 px-3.5 py-2.5 text-[0.86rem] leading-relaxed text-white/85"
                >
                  {{ round.content }}
                </div>
                <AgentToolCard
              ...
              <div v-if="item.error" class="text-[0.78rem] text-red-300">
                ⚠ {{ item.error }}
              </div>
              <div
      ...
      <div class="border-t border-white/10 px-3 py-2.5">
        <div
          class="flex items-end gap-2 rounded-xl border border-white/10 bg-black/25 px-3 py-2 focus-within:border-brand/50"
        >
          <textarea
            ref="inputEl"
            v-model="draft"
            rows="1"
            class="max-h-32 flex-1 resize-none bg-transparent text-[0.86rem] text-white outline-none placeholder:text-white/35"
          ...
          <button
            v-else
            class="inline-flex shrink-0 items-center gap-1 rounded-lg border border-brand/45 bg-brand/14 px-3 py-1.5 text-[0.78rem] text-brand transition-colors hover:bg-brand/24 disabled:opacity-50"
        ...
        <div class="mt-1 px-1 text-[0.66rem] text-white/35">
          助手可以读写沙箱内文件（默认 data/）、执行命令（需审批），技能库：{{ store.defaultDirs?.skillsDir ?? '…' }}
        </div>
      </div>
    </div>
  </div>
